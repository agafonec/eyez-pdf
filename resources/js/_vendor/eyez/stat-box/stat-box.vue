<template>
    ...
    <div v-else-if="variant === 'big'" class="flex items-center w-full relative">

        <div :class="['rounded-full w-[70px] h-[70px] flex items-center justify-center', iconCircleClass]">
            <slot name="icon" />
        </div>
        <div class="mr-4">
            <div v-if="stat.current" class="mb-3">
                <div class="text-sm text-gray-300 leading-tight">{{ stat.current.title }}</div>
                <div class="text-2xl text-gray-900 leading-tight flex">
                    <div>{{ stat.current.value }}</div>
                    <div :class="['text-base mr-2 flex items-center', `${stat.current.value > stat.previous.value ? 'text-green-300' : 'text-red-300'}`]">
                        <span class="font-medium leading-none">{{ percent }}</span>
                        <icon-arrow-up v-if="stat.current.value > stat.previous.value" calss="text-green-300"/>
                        <icon-arrow-down v-else class="text-red-300"/>
                    </div>
            ...
            <div v-if="stat.previous">
                <div class="text-sm text-gray-300 leading-tight">{{ stat.previous.title }}</div>
                <div class="text-2xl text-gray-900 leading-tight">{{ stat.previous.value }}</div>
            </div>
        </div>
    </div>
